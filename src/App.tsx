...
import { 
  ...
  UserCheck, ThumbsUp, UploadCloud, X
} from 'lucide-react';
import { 
  THYROID_CUTS, 
  THYROID_PYQS, 
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-teal-500 selection:text-white">
      ...
      <header className="sticky top-0 z-40 bg-slate-900/90 backdrop-blur-md border-b border-slate-800 px-4 py-2.5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-teal-500 via-indigo-500 to-purple-600 flex items-center justify-center shadow-lg shadow-teal-500/20">
            <Stethoscope className="w-5 h-5 text-white" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="font-extrabold text-lg tracking-tight bg-gradient-to-r from-teal-400 to-indigo-300 bg-clip-text text-transparent">
                MedTutor
              </span>
            ...
            <p className="text-xs text-slate-400 hidden sm:block">NEET-PG & INI-CET Visual Knowledge Engine</p>
          </div>
        </div>
      ...
      {/* ── Active Module Breadcrumb & Status ── */}
      <div className="bg-slate-900/40 border-b border-slate-800/80 px-4 py-2 flex flex-wrap items-center justify-between text-xs text-slate-400">
        <div className="flex items-center gap-2">
      ...
      {/* ── Main Content Area (Multi-Workspace View) ── */}
      <main className="flex-1 overflow-y-auto pb-20">
        {/* ROOM 1: VISUAL CINEMA (DaVinci Modular Cut Engine) */}
        {activeTab === 'visual' && (
          <div className="max-w-7xl mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            {/* Right: 20-Cut Modular DaVinci Beat Sheet Playlist */}
            <div className="lg:col-span-4 flex flex-col gap-3">
              <div className="flex items-center justify-between bg-slate-900 border border-slate-800 rounded-xl p-3">
                <div>
                  <h4 className="text-xs font-bold uppercase tracking-wider text-slate-300">DaVinci Beat Sheet</h4>
        ...
        {activeTab === 'wiki' && (
          <div className="max-w-6xl mx-auto p-4 md:p-6 flex flex-col gap-6">
            ...
            {/* Wiki Card 1: Surgical Anatomy & Ligation Rules */}
            {(wikiSubject === 'all' || wikiSubject === 'anatomy' || wikiSubject === 'surgery') && (
              <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-5 flex flex-col gap-4">
                <div className="flex items-center gap-2 text-sky-400 font-bold text-sm uppercase tracking-wider border-b border-slate-800 pb-2">
                  ...
                  <span>Surgical Anatomy & Nerve Safety Rules</span>
                </div>
            ...
            {/* Wiki Card 2: Pathology Matrix of Thyroiditis */}
            {(wikiSubject === 'all' || wikiSubject === 'patho') && (
              <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-5 flex flex-col gap-4">
                <div className="flex items-center gap-2 text-yellow-400 font-bold text-sm uppercase tracking-wider border-b border-slate-800 pb-2">
                  ...
                  <span>High-Yield Comparison: Thyroiditis Subtypes</span>
                </div>
            ...
            {(wikiSubject === 'all' || wikiSubject === 'patho' || wikiSubject === 'surgery') && (
              <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-5 flex flex-col gap-4">
                <div className="flex items-center gap-2 text-rose-400 font-bold text-sm uppercase tracking-wider border-b border-slate-800 pb-2">
                  ...
                  <span>The Thyroid Oncology Matrix</span>
                </div>
            ...
            {/* Wiki Card 4: Pharmacology & Emergency Protocols */}
        ...
        {/* ROOM 4: CLINICAL ATLAS & COMMUNITY REPOSITORY */}
        {activeTab === 'atlas' && (
          <div className="max-w-6xl mx-auto p-4 md:p-6 flex flex-col gap-6">
            <div className="flex items-center justify-between bg-slate-900 border border-slate-800 p-4 rounded-xl">
              <div>
                <h2 className="text-lg font-black text-white">Clinical Atlas & Verified Case Vault</h2>
                <p className="text-xs text-slate-400">De-identified real-world clinical cases and histopathology slides contributed by doctors.</p>
              </div>
              <button
                onClick={() => setIsCaseUploadModalOpen(true)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-teal-500 hover:bg-teal-400 text-slate-950 text-xs font-bold transition-all shadow-md shadow-teal-500/20 cursor-pointer"
              >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {THYROID_CASES.map(caseItem => (
                <div 
                  key={caseItem.id}
                  className="bg-slate-900/80 border border-slate-800 rounded-xl overflow-hidden shadow-lg flex flex-col justify-between"
                >
                  <div>
                    {/* Case Image */}
                    <div className="relative aspect-video bg-slate-950 overflow-hidden border-b border-slate-800">
                      ...
                      />
                      <div className="absolute top-3 left-3 bg-slate-900/90 text-[10px] font-bold text-teal-300 px-2.5 py-1 rounded border border-slate-700/80 backdrop-blur-sm">
                        {caseItem.ageGender}
                      </div>
                    </div>
                    ...
                    <div className="p-5 flex flex-col gap-3">
                      ...
                      <div className="bg-slate-950 p-3 rounded-lg border border-slate-800/80 text-xs space-y-1.5">
                        <div className="text-slate-300">
                          <strong className="text-slate-400">History:</strong> {caseItem.history}
                        </div>
                        <div className="text-slate-300">
                          <strong className="text-slate-400">Findings:</strong> {caseItem.findings}
                        </div>
                        <div className="text-slate-300">
                          <strong className="text-teal-400">Final Diagnosis:</strong> {caseItem.finalDiagnosis}
                        </div>
                      </div>

                      <div className="space-y-1 pt-1">
                        <span className="text-[11px] uppercase tracking-wider font-bold text-slate-400 block">Management:</span>
                        {caseItem.management.map((step, idx) => (
                  ...
                  {/* Contributor Footer */}
                  <div className="bg-slate-950/90 border-t border-slate-800 p-4 flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="flex items-center gap-1">
                          <span className="font-semibold text-slate-200">{caseItem.contributor.name}</span>
                          {caseItem.contributor.verified && (
                            <span title="Verified Medical Editor">
                              <UserCheck className="w-3.5 h-3.5 text-teal-400" />
                            </span>
                          )}
                        </div>
                        <span className="text-[10px] text-slate-500">{caseItem.contributor.hospital}</span>
                      </div>
                    </div>

                    <button className="flex items-center gap-1 text-slate-400 hover:text-teal-400 transition-colors cursor-pointer">
                      <ThumbsUp className="w-3.5 h-3.5" />
                      <span className="text-xs font-mono">{caseItem.upvotes}</span>
                    </button>
                  </div>
                </div>
              ))}
